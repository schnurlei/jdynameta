...
/**
 * Handle mysql database schemas and creates a schema from a class repository
 *
 * @author rainer
 */
@SuppressWarnings("serial")
public class MsqlSchemaHandler implements Serializable, JdbcSchemaHandler
{
    private final DataSource datasource;

    public MsqlSchemaHandler(DataSource aDatasource) throws JdyPersistentException
    {
    ...
    public boolean existsSchema(String aSchemaName) throws JdyPersistentException
    {
        ...
        try
        {
            ...
            String stmtText = "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME LIKE '" + schemaName + "'";
    ...
    public void deleteSchema(String aSchemaName) throws JdyPersistentException
    {
        ...
        try
        {
            ...
            connection.prepareStatement("DROP SCHEMA IF EXISTS " + schemaName + " ").executeUpdate();
        } catch (SQLException ex)
        {
            throw new JdyPersistentException(ex.getLocalizedMessage(), ex);
        } finally
    ...
    public void createSchema(String aSchemaName, ClassRepository aRepository) throws JdyPersistentException
    {
        ...
        {
            connection = this.datasource.getConnection();
            String createSchemaStmt = "CREATE SCHEMA  IF NOT EXISTS " + schemaName;
            System.out.println(createSchemaStmt);
            connection.prepareStatement(createSchemaStmt).executeUpdate();
            connection.prepareStatement("USE " + schemaName).executeUpdate();
    ...
    {
        String schemaName = aSchemaName.toUpperCase();
        JdbcSchemaValidator validator = new JdbcSchemaValidator(this.datasource.getConnection(), new JDyDefaultRepositoryTableMapping(), new MqSqlTypeHandler(), schemaName);
    ...
    {
        JdbcSchemaValidator validator = new JdbcSchemaValidator(baseConnection, new HsqlUtil.HSqlDbClassInfoToTableMapping(schemaName), new MqSqlTypeHandler(), schemaName);
    ...
    public <TObjToWrite, TReadedObj> JdbcConnection<TObjToWrite, TReadedObj> createBaseConnection(String aSchemaName)
    {
        ...
        JdbcConnection<TObjToWrite, TReadedObj> newBaseConnection
                = new JdbcConnection<>(datasource, null, new HsqlUtil.HSqlDbClassInfoToTableMapping(aSchemaName));
    ...
    {
        SqlTableCreator tableCreator = new HsqlDbJdbcTableCreator(baseConnection, new JDyDefaultRepositoryTableMapping(), new MqSqlTypeHandler());

        try
        {
            baseConnection.prepareStatement("USE " + schemaName).executeUpdate();
