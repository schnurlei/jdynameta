...
import de.jdynameta.metainfoview.filter.AssociationAttributePath;
import de.jdynameta.metainfoview.filter.DeletableOperatorFilterComponent;
...
public class SwingxFilterPanel<TEditObj extends ApplicationObj> extends ManagedPanel
{
	...
	public SwingxFilterPanel(ApplicationManager<TEditObj> anAppMngr, ClassInfo aFilterClassInfo, JdyResourceLoader aResourceLoader, Action refreshAction)
	{
		super();
	...
	{
		AbstractAction createdAction =  new AbstractAction("", resourceLoader.getIcon(ApplicationIcons.RESET_FILTER_ICON))
		{
	...
	private void initUi() 
	{
		...
		this.filterComponentPnl.setBorder(BorderFactory.createEmptyBorder(10, 5, 5, 5));
		this.add(this.filterComponentPnl, BorderLayout.CENTER);
		this.add(createToolBar(), BorderLayout.LINE_START);
	}
	
	private JToolBar createToolBar()
	{
		JToolBar editToolbar = new JToolBar(SwingConstants.VERTICAL);
		editToolbar.setFloatable(false);
		editToolbar.setVisible(true);
		editToolbar.add(createAddComponentAction());
		editToolbar.add(resetAction);
		return editToolbar;
	}
	...
	{
		AbstractAction createdAction = new AbstractAction("",  resources.getIcon(ApplicationIcons.ADD_FILTER_ICON))
		{
	...
	protected void removeComponent(DeletableOperatorFilterComponent<TEditObj> deletableComponent)
	{
		...
		filterPnlCounter--;
		SwingxFilterPanel.this.invalidate();
		SwingxFilterPanel.this.revalidate();
		SwingxFilterPanel.this.repaint();
		if (refreshAction!=null) {
			refreshAction.putValue(BTN_PROPERTY_HIGHLIGHT, "true");
		}
	...
	public static void main(String[] args)
	{
		...
		testFrame.getContentPane().add(new SwingxFilterPanel(null, testClassInfo,resources, null));
		testFrame.pack();
