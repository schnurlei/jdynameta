...
public class HsqlDbAutomaticKeyGenerationTest extends JdbcAutomaticKeyGenerationTest 
{
	...
	public HsqlDbAutomaticKeyGenerationTest(String name)
	{
		super(name);
	}
	...
	{
		return new HsqlDbJdbcTableCreator(aConnection,	new JDyDefaultRepositoryTableMapping(), new JDBCTypeHandler());
	}
	...
	{	
		return new JdyJdbcObjectWriter(aConnection,
				new JDyDefaultRepositoryTableMapping()) {
			@Override
			protected PreparedStatementWriter createPreparedStatementWriter(
					PreparedStatement aPreparedStatement, int aStartIndex) {
				return new HsqlPreparedStatementWriter(aPreparedStatement,
						aStartIndex);
			}
		};
	}
