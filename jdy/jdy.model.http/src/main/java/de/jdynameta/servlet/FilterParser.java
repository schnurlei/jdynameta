...
public final class FilterParser
{
    ...
    /**
     * First convert filter string into byte[]. For LDAP v3, the conversion uses
     * Unicode -> UTF8 For LDAP v2, the conversion uses Unicode -> ISO 8859
     * (Latin-1)
     *
     * Then parse the byte[] as a filter, converting \hh to a single byte, and
     * encoding the resulting filter into the supplied BER buffer
     *
     * @param ber
     * @param filterStr
     * @param isLdapv3
     ...
     */
    public static void encodeFilterString(BerEncoder ber, String filterStr, boolean isLdapv3) throws IOException, NamingException
    {
        ...
        for (filtOffset[0] = filterStart; filtOffset[0] < filterEnd; filtOffset[0]++)
        {
            switch (filter[filtOffset[0]])
            {
                case '(':
                    ...
                    {
                        case '&':
                            encodeComplexFilter(ber, filter, LDAP_FILTER_AND,
                                    filtOffset, filterEnd);
                            parens--;
                            break;

                        case '|':
                            encodeComplexFilter(ber, filter, LDAP_FILTER_OR,
                                    filtOffset, filterEnd);
                            parens--;
                            break;

                        case '!':
                            encodeComplexFilter(ber, filter, LDAP_FILTER_NOT,
                                    filtOffset, filterEnd);
                            parens--;
                            break;
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (escape)
            {
                ...
                if ((ival = hexchar2int(ch)) < 0)
                {
                    ...
                    /**
                     * If there is no hex char following a '\' when parsing a
                     * LDAP v3 filter (illegal by v3 way) we fallback to the way
                     * we unescape in v2.
                     */
                    if (escStart)
                    {
...
//			Ber.dumpBER(System.err, null, answer, 0, j);
            ...
            int filtStart, int filtEnd) throws IOException, NamingException
    {
        ...
        switch (filter[eq - 1])
        {
            ...
            default:
                ...
                {
                    encodeSubstringFilter(ber, filter, typeStart, typeEnd,
                            valueStart, valueEnd);
                    return;
                }
                break;
        ...
        {
            encodeExtensibleMatch(ber, filter, typeStart, typeEnd, valueStart,
                    valueEnd);
        } else
        {
            ber.beginSeq(ftype);
            ber.encodeOctetString(filter, Ber.ASN_OCTET_STR, typeStart, typeEnd
                    - typeStart);
        ...
        while ((index = findUnescaped(filter, '*', previndex, valueEnd)) != -1)
        {
            if (previndex == valueStart)
            {
                if (previndex < index)
                {
                    if (dbg)
                    {
                        System.err.println("initial: " + previndex + ","
                                + index);
                    }
                    ber.encodeOctetString(
    ...
    private static void encodeComplexFilter(BerEncoder ber, byte[] filter,
            int filterType, int filtOffset[], int filtEnd) throws IOException,
            NamingException
    {
            ...
            throws IOException, NamingException
    {
         ...
         * However, for Active Directory interoperability it is transmitted even
         * when FALSE.
         */
