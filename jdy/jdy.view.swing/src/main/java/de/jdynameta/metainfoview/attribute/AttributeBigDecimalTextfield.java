 ...
 *
 */
public abstract class AttributeBigDecimalTextfield implements AttributeComponent 
{
	...
	private final BigDecimal maxInputValue;
	private final BigDecimal minInputValue;
	private long scale;
	...
	public AttributeBigDecimalTextfield(Locale aLocale, BigDecimal aMaxInputValue, long aScale) 
	{
		this(aLocale, new BigDecimal(0), aMaxInputValue, aScale);
	}

	public AttributeBigDecimalTextfield(Locale aLocale, BigDecimal aMinInputValue, BigDecimal aMaxInputValue, long aScale) 
	{
		super();
...
//	    formatter.setMaximum(new Double(maxInputValue.doubleValue()));
	...
	public void writeValueIntoObject()
	{
		if( objectToEdit != null) {
			...
			if (decimal != null) { 
				setBigDecimalInObject(objectToEdit, decimal);
			} else {
				setBigDecimalInObject(objectToEdit, null);
			}
	...
	{		
		...
		if (anObject != null) {
			decimal = getBigDecimalFromObject(anObject);
			if ( decimal != null) {
				decimal = decimal.setScale((int)scale, BigDecimal.ROUND_DOWN);
			}
		}
		originalValue = decimal;
	...
	protected BigDecimal getValue()
	{
		...
		BigDecimal actDecimal = convertDoubleToBigDecimal(actDouble);
	
		return actDecimal;
	}
	

	protected void setValueInTextField(BigDecimal newValue)
	{
		try {
...
//		}
	...
	private BigDecimal convertDoubleToBigDecimal(Double actDouble)
	{
		return (actDouble == null) ? null : new BigDecimal(actDouble.doubleValue()).setScale((int)scale, BigDecimal.ROUND_DOWN);
	}
	...
	protected abstract BigDecimal getBigDecimalFromObject(Object anObject);
	protected abstract void setBigDecimalInObject(Object anObject, BigDecimal insertedDate);
